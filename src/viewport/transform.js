 ...
 *
 * There is no mode: `Konva.Transformer` takes a list of nodes and re-parents
 * nothing, so the selection carries its box. see
 * decisions/transform-control-reparenting, issues/0038.
 *
 * Resizing moves doodads, it does not resize them. A doodad has no scale, so a
 * scaled gizmo would lie about what will be saved. What the anchors are good
 * for is the spacing: `unscaleNodes` keeps the positions the transformer worked
 * out and puts the scale and rotation back, turning a corner drag into "spread
 * this arrangement out". Spacing multiplies, so `bounded` stops a squeeze from
 * reaching zero and staying there.
 *
 * `Box` is the transformer, `Transform` is what it does to a selection of
 * doodads. `viewport/arrays.js` needs the first and none of the second. see
 * issues/0034.
 */
 ...
 *
 * The box measures how far apart the doodads are and a resize multiplies that
 * distance, so doodads squeezed onto one point can never be separated again --
 * every factor of zero is zero, and there is no undo.
 *
 * Konva's own floor of one pixel is no help: the box has already collapsed
 * there. Twenty-four is still worth grabbing an anchor of.
 */
 ...
 *
 * Held on the anchor and not on the box `boundBoxFunc` offers: by the time that
 * is called Konva has already rewritten `_movingAnchorName` past zero, so
 * refusing the box there leaves the transformer holding an anchor the player is
 * not dragging. An anchor that never crosses is a flip that never happens.
 *
 * A box already under the floor -- every box is, zoomed far enough out -- may
 * still be grown but not shrunk, rather than being forced open.
 */
 ...
 * selection's: the same gesture on the same screen. see issues/0034.
 */
const BOX_DEFAULTS = {
  ...
  // Konva's own area is a shape in the overlay layer, above the doodads, so it
  // would swallow the click that takes a doodad back out of the selection.
 ...
 *
 * Konva remeasures for every node whose absolute transform changed, and one pan
 * changes all of them at once -- ten selected doodads measured the box ten
 * times a frame, each walking all ten nodes. see issues/0041. Those asks
 * describe one frame, so the first schedules the work and the rest are it.
 *
 * A frame and not a timer: the callback runs before the browser paints, so the
 * box is drawn where it belongs in the frame the view moved in.
 *
 * The handles need nothing done about the zoom. `Konva.Transformer` overrides
 * `getAbsoluteTransform` to return its own, so it measures and draws in screen
 * pixels however the stage is scaled. Dividing the anchor size by the stage
 * scale is the bug, not the fix.
 */
export class Box extends Konva.Transformer {
   ...
   *
   * The box measures itself a frame at a time, so anything reading where it
   * *is* has to let a scheduled measurement land first.
   */
  space() {
    this.flush();
   ...
   *
   * The box is turned with the view, so "nearer the opposite side" is asked in
   * the box's own space -- `getAbsoluteTransform`, which returns its own
   * transform and nothing above it.
   *
   * The rotate handle is left alone: nothing it does is a collapse.
   */
  inside(point) {
   ...
   *
   * A side anchor leaves one axis alone, so either end of it serves.
   */
  corner(anchor) {
 ...
 * `changed` once it is written back to the domain doodads. The labels follow
 * the doodads across a drag, and a rider reads its motion against where it
 * stood when the gesture began.
 */
export class Transform extends EventTarget {
   ...
   *
   * A rider is moved and not written back: it stands for something that is not
   * a doodad (an array, see `viewport/arrays.js`), so `doodads.apply` has
   * nothing to write into. Hence two lists rather than one with a flag.
   *
   * Riders turn off the anchors. Stretching a selection spreads doodads apart,
   * and resizing an array's box changes its spacing -- one gesture cannot mean
   * both, so while anything is riding the box moves and turns only. see
   ...
   * transformer carry the rest. Any node will do, a rider as readily as a
   * doodad, which is what moves a group holding no doodads at all: a drag is a
   * delta, so the node grabbed does not jump to the pointer.
   */
  startDragging(event) {
   ...
   * gizmos out. The drift accumulates over a drag, nothing putting it back.
   *
   * Moved rigidly, so the spacing this step worked out is unchanged.
   */
  repin() {
